...
@section('content')
    <div class="max-w-md md:max-w-xl mx-auto px-4 my-8 md:my-12">
        ...
        <div class="text-center mb-8">
            <h2 class="font-serif text-3xl md:text-4xl font-medium text-[#1e2e1a] mb-2">Iniciar sesión</h2>
            <p class="text-sm md:text-base text-[#8a8e84]">Accede a tu cuenta de Huellas Felices</p>
        </div>

        <!-- mensaje de estado (reset password, etc) -->
        @if (session('status'))
            <div class="bg-[#eef5e8] text-[#2d5a27] border border-[#c8d9be] text-sm p-3 md:p-4 mb-5 rounded-xl text-center">
                {{ session('status') }}
            </div>
        @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 mb-1.5">
                        <label for="password" class="block text-sm font-medium text-[#1e2e1a]">
                            Contraseña
                        </label>
                        @if(Route::has('password.request'))
                            <a href="{{ route('password.request') }}"
                                class="text-xs text-[#5a9e47] hover:text-[#2d5a27] transition-colors duration-200">
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="mostrarContra('password', 'ojo-login-abierto', 'ojo-login-cerrado')"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-[#8a8e84] hover:text-[#2d5a27] transition-colors duration-200">

                            <!-- ojo abierto -->
                            <svg id="ojo-login-abierto" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                            ...
                            <!-- ojo cerrado -->
                            <svg id="ojo-login-cerrado" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 hidden"
